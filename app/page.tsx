  ...
  return (
    <main className={`world ${entered ? "entered" : ""} ${awakening ? "awakening" : ""}`} data-scene={activeScene}>
      <div className="scroll-track" ref={trackRef}><div className="world-camera" style={{ "--travel": progress } as CSSProperties}>
        ...
        <header className="world-header"><button className="wordmark" onClick={returnHome} aria-label="Return to the beginning"><b className="temp-echo-mark" aria-hidden="true">E</b><span className="wordmark-copy">THE LIVING OCEAN <small>Niue · 19.05°S 169.92°W</small></span></button><button className="method-button" onClick={() => setSourcesOpen(true)}>Sources</button></header>
        ...
        <Scene index={0} active={activeScene === 0} progress={progress} className="entry-scene"><video className="hero-video" src="/media/01-ocean-wall.mp4" autoPlay muted loop playsInline preload="auto" aria-hidden="true" /><div className="scene-image opening-image" aria-hidden="true" /><div className="entry-copy">
        ...
        </div><div className="entry-koru-stage"><button className="enter-button" onClick={enter} aria-label="Enter through the koru and begin the journey through Niue's Living Ocean"><span className="koru-rings" aria-hidden="true" /><Koru /><strong>Touch the Koru to enter</strong></button></div></Scene>
        ...
        <Scene index={1} active={activeScene === 1} progress={progress} className="discovery-scene"><div className="scene-image ocean-change-image" aria-hidden="true" /><div className="scene-current current-warm" aria-hidden="true" /><div className="discovery-copy">
          <p className="eyebrow">01 · Ocean change · observed signal</p><h2>Niue&apos;s ocean<br />is changing.</h2><p className="journey-purpose">This living data journey connects ocean change with village risk, cultural memory and the climate action that could protect what endures.</p>
          <div className="evidence-table ocean-evidence"><SstChart />
            <div className="ocean-projection evidence-row"><strong>{OCEAN_CONTEXT.seaLevelRiseMinCm}–{OCEAN_CONTEXT.seaLevelRiseMaxCm}<small>cm</small></strong><span>projected sea-level rise by {OCEAN_CONTEXT.projectionYear}<em>{OCEAN_CONTEXT.scenario} · {OCEAN_CONTEXT.source}</em></span></div>
            <div className="echo-caption evidence-row"><i /><span><strong>ECHO reads the change</strong>Niue projects increasing ocean acidification under low, medium and high emissions scenarios this century.</span></div>
          </div>
        ...
        <Scene index={2} active={activeScene === 2} progress={progress} className="map-scene"><div className="scene-image loss-damage-image" aria-hidden="true" /><div className="storm-trace" aria-hidden="true"><i /><i /><i /></div><div className="map-intro">
          <p className="eyebrow">02 · Loss &amp; damage · people and place</p><h2>Every number<br />lives somewhere.</h2><p>Touch a village to reveal its verified 2022 census-night population. These counts give risk a human geography; they are not village loss estimates.</p>
          ...
          <aside className="data-signal evidence-table" aria-live="polite"><p className="signal-state">Discovery signal · {selectedVillage === selected.name ? "selected" : "nearby"}</p><h3>{selected.name}</h3>
        ...
        <Scene index={3} active={activeScene === 3} progress={progress} className="memory-scene"><div className="scene-image culture-memory-image" aria-hidden="true" /><div className="memory-orbit" aria-hidden="true"><span className="orbit orbit-one" /><span className="orbit orbit-two" /><span className="memory-number">{EXPOSURE_CONTEXT.censusNightPopulation.toLocaleString("en-US")}</span><span className="memory-label">people counted on census night · 2022</span></div>
          <div className="memory-copy"><p className="eyebrow">03 · Culture &amp; memory · what value cannot hold</p><h2>A place is more than<br />its rebuild cost.</h2><p className="memory-statement">Loss can reach homes, crops, livelihoods, memory and the places that hold belonging.</p>
            <div className="evidence-table memory-evidence"><dl className="exposure-line"><div><dt>{EXPOSURE_CONTEXT.buildings.toLocaleString("en-US")}</dt><dd>buildings in the 2010 exposure inventory</dd></div><div><dt>{EXPOSURE_CONTEXT.majorCropsHectares.toLocaleString("en-US")} ha</dt><dd>major crops mapped</dd></div><div><dt>{money(EXPOSURE_CONTEXT.replacementValueUsd)}</dt><dd>estimated asset replacement value</dd></div></dl>
        ...
        <Scene index={4} active={activeScene === 4} progress={progress} className="future-scene"><div className="scene-image future-image" aria-hidden="true" /><div className="finance-paths" aria-hidden="true"><i /><i /><i /><i /></div><div className="future-content">
          <p className="eyebrow">04 · Climate finance · the choice ahead</p><h2>Recognition is not<br />resilience.</h2><div className="finance-evidence evidence-table"><div className="finance-journey" aria-label="The climate finance pathway"><span>Need recognised</span><i /><span>Action costed</span><i /><span>Delivery conditional</span></div><div className="finance-focus"><p>One possible response · Niue NDC 3.0</p><strong>{money(FINANCE_CONTEXT.amountUsd)}</strong><span>{FINANCE_CONTEXT.action}</span><em>{FINANCE_CONTEXT.status} · stated need, not confirmed expenditure</em></div></div>
